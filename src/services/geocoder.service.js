...
export default {
        ...
        return new Promise((resolve, reject) => {
            ...
            geocoder.geocode({'address': address}, (results, status) => {
                if (status === google.maps.GeocoderStatus.OK) {
                    ...
                    let { address_components } = location;
                    resolve({
                        address_components: {
                            address: `${this.extractFromAddress(address_components, 'street_number')} ${this.extractFromAddress(address_components, 'route')}`,
                            city: `${this.extractFromAddress(address_components, 'locality')}`,
                            county: `${this.extractFromAddress(address_components, 'administrative_area_level_2')}`,
                            state: `${this.extractFromAddress(address_components, 'administrative_area_level_1')}`,
                            postal_code: `${this.extractFromAddress(address_components, 'postal_code')}`,
                        },
                        location: {
    ...
    extractFromAddress(components, type) {
        return components.filter((component) => component.types.includes(type)).map((item) => item.long_name).pop() || null;
    }
};