...
export default function useConnectFirst(context: { emit: Function }) {
    ...
    const isNotTakingCalls = computed(() => store.getters["phone/isNotTakingCalls"]);
    const isOnCall = computed(() => store.getters["phone/isOnCall"]);
    const isTransitioning = computed(() => store.getters["phone/isTransitioning"]);
    ...
    const setCallHistory = (callHistory: any) => store.commit("phone/setCallHistory", callHistory);
    const setCallType = (callType: any) => store.commit("phone/setCallType", callType);
    const setCaller = (caller: any) => store.commit("phone/setCaller", caller);
    const setOutgoingCall = (call: any) => store.commit("phone/setOutgoingCall", call);
    const setCurrentCall = (call: any) => store.commit("phone/setCurrentCall", call);
    ...
    async function loginPhone(retry = true, status = 'WORKING') {
        ...
        if (!languages?.value?.length) {
            await $toasted.error(
                t('phoneDashboard.select_language_error'),
            );
            return;
        }
        ...
        if (!currentUser?.value?.mobile) {
            await $toasted.error(
                t('phoneDashboard.add_phone_number_error'),
            );
            return;
        }
    ...
    async function createOutboundCall(outbound: any, number: string) {
        const dnisResponse = await axios.get(
            `${import.meta.env.VUE_APP_API_BASE_URL}/phone_dnis/${outbound.dnis1}`,
        );
    ...
    return {
        ...
        isInboundCall,
        isOutboundCall,
        setCurrentIncidentId: (id: string) => store.commit("incident/setCurrentIncidentId", id),
        setOutgoingCall,
        ...
        setCallType,
        setCallHistory,
        setGeneralStats: (stats: any) => store.commit("phone/setGeneralStats", stats),
