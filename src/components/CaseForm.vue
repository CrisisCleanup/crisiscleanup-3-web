<template>
    <form ref="form" @submit.prevent="handleSubmit" layout="vertical" class="bg-white flex flex-col flex-grow">
        <div class="intake-form p-3 flex-grow">
            ...
            <div class="py-1">
                <base-input :value="worksite.what3words" @input="(value) => { updateWorksite(value,'what3words') }"
                            tooltip="info" size="large" placeholder="Location" required disabled></base-input>

                <div class="flex justify-around items-center">
                    <base-button type="bare" size="large" icon="street-view" class="text-gray-700 pt-2"
                                 :action="locateMe"
                                 text="Use my location"/>
                    <base-button type="bare" size="large" icon="map" class="text-gray-700 pt-2" :action="showOverlayMap"
                                 text="Select on Map"/>
                    <modal v-if="overlayMapVisible" @close="overlayMapVisible = false"
            ...
            <div class="py-1">
                <base-input :value="worksite.city" @input="(value) => { updateWorksite(value,'city') }" tooltip="info"
                            size="large" placeholder="City" required @change="findPotentialGeocode"></base-input>
            </div>
            <div class="py-1">
                <base-input :value="worksite.county" @input="(value) => { updateWorksite(value,'county') }"
                            tooltip="info" size="large" placeholder="County" required
                            @change="findPotentialGeocode"></base-input>
            </div>
            <div class="py-1">
                <base-input :value="worksite.state" @input="(value) => { updateWorksite(value,'state') }" tooltip="info"
                            size="large" placeholder="State" required @change="findPotentialGeocode"></base-input>
            </div>
            <div class="py-1">
                <base-input :value="worksite.postal_code" tooltip="info" size="large" placeholder="Postal Code" required
                            @change="findPotentialGeocode"></base-input>
            </div>
                ...
                <div :key="field.field_key" v-if="showAllFields || getValue(field.field_key)">
                    <template v-if="['h4'].includes(field.html_type)">
                        <component :is="field.html_type" :key="field.field_key"
                            ...
                            <div class="flex items-center">
                                <badge class="mr-2 bg-primary-light text-black" width="20px" height="20px">
                                    {{field.list_order}}
                                </badge>
                                {{field.label_t}}
                            </div>
                    ...
                    <template v-if="field.html_type === 'checkbox'">
                        <div class="py-1 flex items-center" :key="field.field_key">
                            <base-checkbox :value="getBooleanValue(field.field_key)"
                                           v-model="dynamicFields[field.field_key]">{{field.label_t}}
                            </base-checkbox>
                            <a-tooltip class="px-1">
                                <template slot="title">
                                    <span v-html="field.help_t"></span>
        ...
        <div class="bg-white p-3 border border-r-0 border-gray-300 card-footer flex justify-between">
            <base-button size="medium" class="flex-grow m-1 border-2 border-black"
                         :action="() => { $emit('closeWorksite') }" text="Cancel"/>
            ...
            async saveWorksite(reload = true) {
                ...
                try {
                    if (this.worksite.id) {
                        ...
                        let savedWorksite = await Worksite.api().post('/worksites', {
                            ...this.worksite,
                            skip_duplicate_check: true
                        });
